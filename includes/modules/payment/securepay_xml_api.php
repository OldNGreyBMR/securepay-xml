...
class securepay_xml_transaction
{
    ...
    {
        if (
            array_key_exists("txnResult", $this->responseArray) === true
    ...
    private function processTransaction()
    {
        ...
        // check parameters
        if (
            $this->getTxnType() == SECUREPAY_TXN_STANDARD ||
    ...
    private function checkCCParameters()
    {
        ...
        if (
            strlen($this->getCCNumber()) < 12 ||
            strlen($this->getCCNumber()) > 19 ||
            preg_match("/\D/", $this->getCCNumber())
        )//Match non-digit
        {
        ...
        if (
            preg_match("/\D/", $this->getCCExpiryMonth()) ||
        ...
        if (
            preg_match("/\D/", $this->getCCExpiryYear()) ||
        ...
        if ($this->getCCVerify() != false) {
            ...
            if (
                preg_match("/\D/", $this->getCCVerify()) || // REGEXP: true if "any match for non-numeral"
    ...
    private function checkTxnParameters()
    {
        ...
        if (
            ...
            if (
                strlen($ref) == 0 || strlen($ref) > 18 ||
                preg_match('/[^0-9a-zA-Z*\.&\/-_\']/', $ref)
            ) // REGEXP: match any non-EBCDIC character
            {
            ...
            if (
                strlen($ref) == 0 || strlen($ref) > 60 ||
                preg_match('/[^ \']/', $ref) == false
            ) // REGEXP: match invalid characters
            {
                $this->errorString = self::GATEWAY_ERROR_INVALID_REF_ID;
                return false;
            }
    ...
    private function createXMLTransactionRequestString()
    {
        ...
        if (
            $this->getTxnType() == SECUREPAY_TXN_REFUND ||
    ...
    private function processTransactionResponseMessageIntoResponseArray(string $responseMessage): bool
    {
        ...
        if ($this->getRequestType() == SECUREPAY_REQ_PERIODIC) {
            ...
            strtoupper($responseArray['approved']) == 'YES' &&
            ($this->getTxnType() == SECUREPAY_TXN_DIRECTDEBIT ||
                $this->getTxnType() == SECUREPAY_TXN_DIRECTCREDIT)
        ) {
        ...
        if (
            (strcasecmp($responseArray["approved"], "Yes") == 0) &&
