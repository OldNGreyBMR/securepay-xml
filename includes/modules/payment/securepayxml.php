 ...
 */
// BMH  2025-07-18 add version number; PHP8.4 compatible

declare(strict_types=1);
if (!defined('VERSION_SECUREPAYXML')) {
    define('VERSION_SECUREPAYXML', '1.5.8');
}
...
class securepayxml
{
    ...
    function __construct()
    {
        ...
        $this->enabled = defined('MODULE_PAYMENT_SECUREPAYXML_STATUS') && (MODULE_PAYMENT_SECUREPAYXML_STATUS == 'True');
        $this->sort_order = defined('MODULE_PAYMENT_SECUREPAYXML_SORT_ORDER') ? ((int) MODULE_PAYMENT_SECUREPAYXML_SORT_ORDER) : null;
        ...
        $this->mode = defined('SECUREPAY_GATEWAY_MODE_TEST');
        if (defined('MODULE_PAYMENT_SECUREPAYXML_TEST') == "No") {
            $this->mode = SECUREPAY_GATEWAY_MODE_LIVE;
        }

        if ((int) defined('MODULE_PAYMENT_SECUREPAYXML_ORDER_STATUS_ID') > 0) {
            $this->order_status = MODULE_PAYMENT_SECUREPAYXML_ORDER_STATUS_ID;
        }

        if (defined('MODULE_PAYMENT_SECUREPAYXML_MODE') == defined('MODULE_PAYMENT_SECUREPAYXML_MODE_PREAUTH') && (int) defined('MODULE_PAYMENT_SECUREPAYXML_PREAUTH_ORDER_STATUS_ID') > 0) {
            $this->order_status = MODULE_PAYMENT_SECUREPAYXML_PREAUTH_ORDER_STATUS_ID;
        }

        $this->zone = (int) defined('MODULE_PAYMENT_SECUREPAYXML_ZONE');
        ...
        if (defined('MODULE_PAYMENT_SECUREPAYXML_STATUS') == 'True') {
        ...
        $confirmation = array(
            ...
            'fields' => array(
                ...
                array(
                    ...
                    'field' => date('F, Y', mktime(0, 0, 0, $_POST['securepayxml_cc_expires_month'], 1, (int) ('20' . $_POST['securepayxml_cc_expires_year'])))
                )
    ...
    public function before_process()
    {
        ...
        if ($type == SECUREPAY_TXN_PREAUTH) {
            $result = $sxml->processCreditPreauth($amount, (string) $oid, $cc_number, $cc_month, $cc_year, $cvv);
        } else {
            // BMH debug
            // BMH DEBUG echo 'securepayxml line 331 $oid=' . $oid; // BMH
            $result = $sxml->processCreditStandard($amount, (string) $oid, $cc_number, $cc_month, $cc_year, $cvv);
   ...
   public function install()
     ...
     */
    public function _updateOrderStatus($oID, $new_order_status, $comments)
    {
        global $db;
        $sql_data_array = array(
     ...
     */
    public function _updateTxn($oid, $amount = 0, $txn = -1, $type = -1)
    {
        global $db;
    ...
    public function _doRefund($oID, $amount = 0)
    {
        ...
        return false;  // Is this code unreachable // BMH ??
    }
     ...
     */
    public function _doCapt($oID, $amt = 0, $currency = 'AUD')
    {
